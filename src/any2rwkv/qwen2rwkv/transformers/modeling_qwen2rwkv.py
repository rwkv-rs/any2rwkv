    ...
    def initialize_gqa_sidecar(
        ...
        *,
        ...
        if self.gqa_keys is None:
            ...
            self.gqa_positions = torch.full(
                (batch, capacity), -1, dtype=torch.int32, device=device
            )
        elif (
...
def _admit_sidecar_token(
    ...
    *,
    ...
    if any(value is None for value in provided) and not all(
    ...
    evicted_position = torch.full(
        (batch,), -1, dtype=torch.int32, device=positions.device
    )
    ...
    if position < sink_slots:
        ...
        if position < capacity:
            ...
            keep_candidate = (candidate_score > heavy_score) | (
                (candidate_score == heavy_score)
                & (candidate_position > selected_heavy_position)
            )
...
def _validate_sidecar_plan(
    ...
    bounded_positions = (~valid) | (
        (snapshots >= 0) & (snapshots <= token_index)
    )
    ...
    expected_evictions = (
        torch.arange(1, length + 1, dtype=torch.int32, device=snapshots.device)
        - capacity
    ).clamp_min(0)
...
def _validate_live_sidecar(
    ...
    valid = positions >= 0
    sorted_positions = positions.masked_fill(
        ~valid, torch.iinfo(torch.int32).max
    ).sort(-1).values
    duplicate = (
        (sorted_positions[:, 1:] == sorted_positions[:, :-1])
        & (sorted_positions[:, 1:] != torch.iinfo(torch.int32).max)
...
class Qwen2RWKVTimeMix(nn.Module):
    ...
    def __init__(self, config: Qwen2RWKVConfig, layer_idx: int):
        ...
        self.beta_logit = nn.Parameter(
            torch.full((self.num_heads,), math.log(0.1 / 0.9))
        )
    ...
    def _feature(self, value: torch.Tensor, weight: torch.Tensor) -> torch.Tensor:
        projected = torch.einsum(
            "bhtd,hdf->bhtf", value.float(), weight.float()
        )
        feature = torch.cat(
    ...
    def _sidecar_plan(
        ...
        scores = torch.zeros(
            batch, self.sidecar_capacity, dtype=torch.float32, device=query.device
        )
        ...
        evicted = torch.full(
            (batch, length), -1, dtype=torch.int32, device=query.device
        )
        ...
        for token in range(length):
            ...
            slot_index = slot_index[:, None, :, None].expand(
                -1, self.num_heads, -1, self.head_size
            )
            exact_key = torch.gather(key_heads, 2, slot_index)
            logits = torch.einsum(
                "bhd,bhsd->bhs", query[:, :, token].float(), exact_key.float()
            ) * self.scaling
            ...
            linear_denominator = (
                feature_query[:, :, token].float() * tail_feature_sum
            ).sum(-1)
            raw_denominator = (
                beta * exact_denominator + alpha * linear_denominator
            )
    ...
    def _eviction_stream(
        ...
        value_index = value_index[:, None, :, None].expand(
            -1, self.num_heads, -1, self.head_size
        )
    ...
    def _tail_reference(
        ...
        write_key, write_value, valid = self._eviction_stream(
            feature_key, value_heads, evicted
        )
        ...
        for token in range(length):
            ...
            numerator[:, :, token] = torch.einsum(
                "bhdf,bhf->bhd", numerator_state, read
            )
            denominator[:, :, token] = torch.einsum(
                "bhf,bhf->bh", denominator_state, read
            )
    ...
    def _tail_training(
        ...
        query_padded = F.pad(
            feature_query, (0, self.head_size - self.feature_output_dim)
        ).permute(0, 2, 1, 3)
        key_padded = F.pad(
            write_key, (0, self.head_size - self.feature_output_dim)
        ).permute(0, 2, 1, 3)
    ...
    def _combine(
        ...
        for token in range(query.shape[2]):
            ...
            exact_numerator = torch.einsum(
                "bhs,bhsd->bhd", kernel, exact_value.float()
            )
            raw_denominator = (
                beta * exact_denominator
                + alpha * tail_denominator[:, :, token].float()
            )
            denominators.append(raw_denominator)
            denominator = raw_denominator.clamp_min(1e-12)
            output = (
            ...
            exact_mass = exact_mass + (
                beta * exact_denominator / denominator
            ).mean()
            linear_mass = linear_mass + (
                alpha * tail_denominator[:, :, token].float() / denominator
            ).mean()
            exact_probability = beta[..., None] * kernel / denominator[..., None]
            sink_mass = sink_mass + exact_probability[..., : self.sink_slots].sum(-1).mean()
            recent_mass = recent_mass + exact_probability[
                ..., self.sink_slots : self.sink_slots + self.recent_slots
            ].sum(-1).mean()
            heavy_mass = heavy_mass + exact_probability[
                ..., self.sink_slots + self.recent_slots :
            ].sum(-1).mean()
        _require_positive_finite(
        ...
        self._last_sidecar_metrics = {
            ...
            "recent_count": final_valid[
                :, self.sink_slots : self.sink_slots + self.recent_slots
            ].sum(-1).float().mean(),
            "heavy_count": final_valid[
                :, self.sink_slots + self.recent_slots :
            ].sum(-1).float().mean(),
        }
    ...
    def attention_heads_reference(
        ...
        snapshots, evicted = self._sidecar_plan(
            query, key, feature_query, feature_key
        )
        value_heads = _repeat_gqa(value, self.num_kv_groups)
        ...
        return (
            self._combine(
                query, key, value, snapshots, numerator, denominator
            ),
            gate,
        )
    ...
    def _inference_forward(
        ...
        for offset in range(x.shape[1]):
            ...
            valid_eviction = evicted_position >= 0
            evicted_key_heads = _repeat_gqa(
                evicted_key.unsqueeze(2), self.num_kv_groups
            )
            write_key = self._feature(
                evicted_key_heads, self.feature_k_weight
            ).squeeze(2)
            ...
            value_heads = _repeat_gqa(
                evicted_value.unsqueeze(2), self.num_kv_groups
            ).squeeze(2)
            value_heads = value_heads * valid_eviction[:, None, None]
            denominator_value = torch.ones_like(value_heads) * valid_eviction[
                :, None, None
            ]
            read = torch.stack((read, read), dim=2).reshape(
                x.shape[0], self.kernel_heads, self.head_size
            ).contiguous()
            write_key = torch.stack((write_key, write_key), dim=2).reshape_as(
                read
            ).contiguous()
            write_value = torch.stack(
                (value_heads, denominator_value), dim=2
            ).reshape_as(read).contiguous()
            decay = torch.full_like(read, GQA_TAIL_DECAY_LOGITS)
            erase = torch.zeros_like(read)
            offsets, indices, ticket = cache.recurrent_metadata(
                flash, x.shape[0], 1, x.device
            )
            raw = flash.infer_recurrent_fp16_forward_varlen(
                read,
                decay,
            ...
            gather = slot_index[:, None, :, None].expand(
            ...
            raw_denominator = (
                beta * exact_denominator + alpha * tail_denominator
            )
            denominators.append(raw_denominator)
            denominator = raw_denominator.clamp_min(1e-12)
            output = (
                beta[..., None] * exact_numerator
                + alpha[..., None] * tail_numerator
            ) / denominator[..., None]
            ...
            linear_mass = linear_mass + (
                alpha * tail_denominator / denominator
            ).mean()
            sink_mass = sink_mass + exact_probability[..., : self.sink_slots].sum(-1).mean()
            recent_mass = recent_mass + exact_probability[
                ..., self.sink_slots : self.sink_slots + self.recent_slots
