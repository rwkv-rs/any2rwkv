...
def _initialize_layer(source_text, student, layer_idx, init_hidden, validation_hidden):
    ...
    if source_text.config.layer_types[layer_idx] == "linear_attention":
        ...
        with torch.no_grad():
            ...
            probe_embeddings = tuple(value[:1, :probe_length] for value in init_embeddings)
            ...
            actual_block = (
                residual + target_layer.mlp(target_layer.post_attention_layernorm(residual))
            ).float()
            block_nmse = float(
...
def _evaluate_gqa_reference(source_text, student, layer_idx, hidden, world, device):
    ...
    try:
        reference_layer_module = (
            copy.deepcopy(target_layer).to(device=device, dtype=torch.float32).eval()
        )
    finally:
    ...
    for batch in hidden.split(8):
        ...
        reference_attention, reference_gate = reference_layer_module.tmix.attention_heads_reference(
            normalized
        )
        ...
        def complete(layer, layer_input, tmix_output):
            ...
            return residual + layer.mlp(layer.post_attention_layernorm(residual))

        reference_layer = complete(reference_layer_module, reference_batch, reference_tmix).float()
        native_layer = complete(target_layer, batch, native_tmix).float()
        wanted_layer = complete(source_layer, batch, wanted_tmix).float()
        pairs = (
...
def _evaluate_gqa_fp16_cache(
    ...
    try:
        runtime_layer = (
            copy.deepcopy(source_layer_copy).to(device=device, dtype=torch.float16).eval()
        )
    finally:
    ...
    for batch in hidden.split(8):
        ...
        wanted_tmix = source_layer.self_attn.o_proj(
            wanted_attention_bt.reshape(*batch.shape) * torch.sigmoid(gate.reshape(*batch.shape))
        )
    ...
    _, _, _, reordered_cache = _fp16_forward_mode(runtime_layer, prefix, student.config, 64)
    reordered_cache.batch_repeat_interleave(2)
    reordered_cache.batch_select_indices(torch.tensor([1, 2], dtype=torch.long, device=device))
    ...
    for name in pair_names[3:]:
        if metrics[f"{name}_nmse"] > 1e-3:
            raise RuntimeError(
                f"FP16 cache parity gate failed for {name}: {metrics[f'{name}_nmse']:.8g} > 0.001"
            )
    ...
    *,
    ...
    for epoch in range(epochs):
        ...
        for training_batch in loader:
            ...
            exact_attention = (
                training_batch[1].to(device) if phase == "attention_transfer" else None
            )
            with torch.no_grad():
                if phase == "low_rank_correction":
                    wanted_tmix = _teacher_tmix_output(source_text, layer_idx, hidden)
                    source_layer = source_text.layers[layer_idx]
                    ...
                    wanted_tmix = None
                    wanted_layer = None
            actual_layer, actual_tmix, attention_loss = wrapper(hidden, exact_attention)
            if phase == "attention_transfer":
                loss = attention_loss
                block_loss = loss.detach().new_zeros(())
                tmix_loss = loss.detach().new_zeros(())
            else:
                block_loss = (actual_layer.float() - wanted_layer.float()).square().mean() / (
                    wanted_layer.float().square().mean() + 1e-6
                )
                tmix_loss = (actual_tmix.float() - wanted_tmix.float()).square().mean() / (
...
def _align_gqa_layer(
    ...
    try:
        ...
        sidecar = {name: float(value) for name, value in tmix._last_sidecar_metrics.items()}
        ...
        if world == 1 or dist.get_rank() == 0:
            ...
            for local_name in (
                ...
                if values is not None:
                    failure.update(
                        {f"{local_name}_{name}": value for name, value in values.items()}
                    )
    ...
    *,
    ...
    for index in range(completed, through_layer + 1):
        ...
        if not is_gdn:
            ...
            layer_strict_pass = validation_metrics["layer_output_nmse"] <= LAYER_OUTPUT_NMSE_TARGET
            if prefix_cache is None:
